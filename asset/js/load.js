...
function load_service(){
    ...
    $.getJSON('/api.php?type=service', function(data){
        ...
        for(let i = 0; i < service.length; i++){
            ...
            div.append('<td class="text-center">' + (service[i]['status'] == 1 ? '<span class="text-success"><i class="fas fa-check-circle"></i> Hoạt động</span>' : 'Bảo trì') +'</td>');
            div.append('<td class="text-center">' + (service[i]['ping'] != -1 ? '<span class="text-success"><i class="fas fa-wifi"></i> ' + service[i]['ping'] + 'ms</span>' : '<i class="fas fa-times" style="color: red"></i>') +'</td>');
            div.append('</tr>');
        }
    });
}
